...
class Imager(object):
    ...
    def doSingleFrame(self):
        ...
        if self.dark:
            ...
            self.client.executeCommand(
                SetAcquisitionType(0))  # LIGHT, BUFFER 1
    ...
    def doFrameTransfer(self):
        ...
        if self.dark:
            self.client.executeCommand(
                SetAcquisitionType(1, 1))  # BUFFER 1, DARK
        else:
            self.client.executeCommand(
                SetAcquisitionType(1, 0))  # BUFFER 1, LIGHT

        self.client.executeCommand(
            SetMultipleFrameBufferMode(1)),  # GET ALL FRAMES
        ...
        comments = []
        comments.append(
            ("NOTE1", "WARNING!!! The DATE-OBS and DATE-END headers are accurate "))
        comments.append(
            ("NOTE2", "WARNING!!! only for the first frame of a sequence."))
        comments.append(
            ("NOTE3", "WARNING!!! Other frames use an estimated value based "))
        comments.append(
            ("NOTE4", "WARNING!!! on the exposure time of the sequence."))
        ...
        for run in sets:
            ...
            for i in range(run):
                ...
                new_array = img_array.reshape(
                    parallel_length / run, serial_length)
                ...
                self._saveFITS(
                    new_array, date_headers + comments, self._getNextName())
                del new_array
    ...
    def getCameraPars(self):
        ...
        xcpars.seek(0)
        a = np.loadtxt(xcpars, delimiter=',', dtype=[
                       ('group', 'S30'), ('name', 'S30'), ('value', 'int')])
        print '->', a
    ...
    def _updateDateHeader(self, orig_header, idx):
        ...
        start_time = time.mktime((
            ut.tm_year, ut.tm_mon, ut.tm_mday, start_hour, start_min, int(start_sec), ut.tm_wday, ut.tm_yday, ut.tm_isdst))
        ...
        end_time = time.mktime(
            (ut.tm_year, ut.tm_mon, ut.tm_mday, end_hour, end_min, int(end_sec), ut.tm_wday, ut.tm_yday, ut.tm_isdst))

        fits_date_format = "%Y-%m-%dT%H:%M:%S.%%03d"
        date = time.strftime(fits_date_format, time.gmtime())
        date = date % 0

        dateobs = time.strftime(
            fits_date_format, time.gmtime(start_time + (idx * (self.texp + 1.0))))
        dateobs = dateobs % ((start_sec - int(start_sec)) * 1000)

        dateend = time.strftime(
            fits_date_format, time.gmtime(end_time + (idx * (self.texp + 1.0))))
